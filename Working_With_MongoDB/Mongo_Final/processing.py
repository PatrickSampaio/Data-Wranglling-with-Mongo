...
DATAFILE = '../../fixtures/arachnid.csv'
...
PARENTHISIS_PATTERN = re.compile("^.*\(.*\)$")
NON_ALPHANUMERIC_PATTERN = re.compile(r'^\W|\w+$')
CHECK_FOR_E_PATTERN = re.compile("^.*\&.*$")
CHECK_FOR_PIPE_PATTERN = re.compile("^.*\*\|.*$")

def clean_field(value, field):

    if field == "rdf-schema#label" or field == "name":
        if PARENTHISIS_PATTERN.match(value) is not None:
            value = value.split("(")[0].strip()

    #if field == "name":
    #     if NON_ALPHANUMERIC_PATTERN.match(value) is not None:
    #        value = value[1:]
